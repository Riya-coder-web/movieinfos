...
async function fetchMovies(query) {
    try {
        sessionStorage.setItem('searchQuery', query); // Store query in session storage
...
// Check for saved search query on page load
window.onload = function() {
    const savedQuery = sessionStorage.getItem('searchQuery');
    if (savedQuery) {
        searchInput.value = savedQuery; // Set the search input value to saved query
        fetchMovies(savedQuery); // Fetch movies using the saved query
    }
};
...
function displayMovies(movies, isWatchlist) {
    movieGrid.innerHTML = '';
...
document.getElementById('arrow-to-main').addEventListener('click', function() {
